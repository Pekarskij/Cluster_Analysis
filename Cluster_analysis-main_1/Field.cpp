...
void Field::add_cloud_in_field(Cloud c)
{
    ...
    clouds_in_field[number_clouds_in_field - 1].assign_cloud_id(number_clouds_in_field - 1);//numeruem dobavlennoe oblako
    for (i = 0; i < clouds_in_field[number_clouds_in_field - 1].get_number_points_in_cloud(); i++)//numeruem tochki v dobavlennom oblake
    {
...
void Field::delete_cloud_from_field(int id)//udalyaem oblako s nomerom id
{
    int i, j, k = 0;
...
int Field:: get_number_points_in_field()
{
    return number_points_in_field;
}

int Field::get_number_clouds_in_field()
{
    return number_clouds_in_field;
}
...
Point Field::get_point_by_id(int id)
{
    int n = 0;
    Point p;
    for (int i = 0; i < number_clouds_in_field; i++)
    {
        n += clouds_in_field[i].get_number_points_in_cloud();
        if (id < n)
        {
            p =  clouds_in_field[i].get_point_in_cloud(id);
        }
    }
    return p;
}
vector<double> Field::get_dist_mat()
{
    return dist_mat;
}
void Field::create_dist_mat()
{
    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            ...
            dist_mat.push_back((points[i] - points[j]).length());
        }
    }
    points.clear();
}
...
{
    int n = buffer.get_number_clouds_in_buffer();
    for (int i = 0; i < n; i++) add_cloud_in_field(buffer.upload_cloud_from_buffer(n - i - 1));
}
