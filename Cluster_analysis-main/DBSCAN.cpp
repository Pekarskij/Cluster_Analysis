...
Find_cluster DBSCAN::find_clusters(vector <Point>& points, vector <int>& cpd) //cpd = 0 -osnovnaya tochka, cpd = 1 - tochka periferii, cpd = 2 - shum
{
    int i, j, l, t, c, N = 0, n = points.size(), s = 0;
   vector <double> D, D_core;
   vector <int> B, B_core;
    ...
    cpd.resize(n);
    D = fc.distance_matrix(points);
    B = fc.graph_matrix(points, delta);
