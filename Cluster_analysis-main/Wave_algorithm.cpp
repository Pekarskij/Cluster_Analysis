...
Find_cluster Wave_algorithm::find_clusters(vector <Point>& points)
{
    int i, j, k, t = 0, c, n = points.size(), s = 0;
    vector <vector <int>> point_indicators(n);
    vector <Cluster> C;
    Find_cluster fc;
    vector <double> D = fc.distance_matrix(points);
    vector <int> B = fc.graph_matrix(points, delta);
    ...
    for (i = 0; i < n; i++)//point_indicators[i][1] == 1/0 - i-tay tochka v clustere/ne v clustere, point_indicators[i][0] - metka i-toy tochki
    {
