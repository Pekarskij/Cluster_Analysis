...
Cloud::Cloud(double mean_x, double mean_y, double sigma_x, double sigma_y, int n)
{
    ...
    if (number_points_in_cloud != 0)
    {
        for (int i = 0; i < number_points_in_cloud; i++)
        {
            ...
            sum_x = sum_x * 0.001 * dispersion_for_X + mean_value_for_X;
            sum_y = sum_y * 0.001 * dispersion_for_Y + mean_value_for_Y;
            points_in_cloud.push_back(Point(sum_x, sum_y));
            sum_x = 0;
            sum_y = 0;
        }
    }
    for (int i = 0; i < number_points_in_cloud; i++) points_in_cloud[i].assign_Point_id(i);
    for (int i = 0; i < number_points_in_cloud; i++)
    {
...
void Cloud::turn_cloud(double phi)
{
    ...
    for (int i = 0; i < number_points_in_cloud; i++)
    {
        x = cos(phi) * (points_in_cloud[i].GetX() - mean_value_for_X) - sin(phi) * (points_in_cloud[i].GetY() - mean_value_for_Y);
        y = sin(phi) * (points_in_cloud[i].GetX() - mean_value_for_X) - cos(phi) * (points_in_cloud[i].GetY() - mean_value_for_Y);
